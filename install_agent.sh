...
if [ "$(id -u)" -ne 0 ]; then
  ...
  echo "   sudo bash $0 --domain <domain> --endpoint <url>"
  exit 1
fi
...
usage() {
  cat <<EOF
Usage:
  sudo bash install_agent.sh --domain vk.com --endpoint https://example.com/api/ingest [--iface eth0] [--interval 60]

Options:
  --domain     Domain/name to tag this node (sent as server_domain)
...
if [[ -z "$DOMAIN" || -z "$ENDPOINT" ]]; then
  echo "❌ Нужно указать --domain и --endpoint"
  usage
  exit 1
fi
...
def get_primary_local_ip() -> str:
    """
    Нормально работает даже без интернета: определяет IP, через который машина ходит "наружу"
...
def read_net_bytes(interface: Optional[str]) -> Tuple[int, int]:
    ...
    if interface:
        ...
        if nic is None:
            raise RuntimeError(f"Network interface '{interface}' not found. Available: {', '.join(sorted(pernic.keys()))}")
...
def main() -> None:
    cfg = Config(
        server_domain=get_env_required("TRAFFIC_AGENT_DOMAIN"),
    ...
    # CPU: нужно "прогреть" первый вызов, чтобы следующий был % за интервал
    ...
    while True:
        now_ts = time.time()
        # timestamp в unix seconds (int)
        ts_int = int(now_ts)

        # CPU % за время с предыдущего вызова cpu_percent()
        ...
        try:
            ...
            # не падаем, просто пишем в stdout -> journald
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] отправка не удалась: {e} payload={payload}")

        prev_rx, prev_tx = rx, tx
        prev_ts = now_ts

        # добиваемся ровного интервала, учитывая время работы цикла
...
Restart=always
RestartSec=5
# чуть адекватнее лимиты логов по умолчанию в journald, но тут без излишеств
...
systemctl enable traffic_snapshots_agent.service
systemctl restart traffic_snapshots_agent.service
systemctl status traffic_snapshots_agent
